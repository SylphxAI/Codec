...
import { decodeTta, encodeTta, isTta, parseTtaInfo } from './index'
import type { TtaAudioData } from './types'
...
describe('TTA Codec', () => {
	...
	describe('isTta', () => {
		it('should identify TTA files', () => {
			const audio = createSineWave(44100, 0.1, 440, 2, 16)
			const tta = encodeTta(audio)
			expect(isTta(tta)).toBe(true)
		})

		it('should reject non-TTA files', () => {
			expect(isTta(new Uint8Array([0, 0, 0, 0, 0, 0, 0, 0]))).toBe(false)
			expect(isTta(new Uint8Array([0x89, 0x50, 0x4e, 0x47]))).toBe(false) // PNG
			expect(isTta(new Uint8Array([0x66, 0x4c, 0x61, 0x43]))).toBe(false) // FLAC
		})

		it('should handle short data', () => {
			expect(isTta(new Uint8Array([]))).toBe(false)
			expect(isTta(new Uint8Array([0x54, 0x54]))).toBe(false)
		})
	})

	describe('parseTtaInfo', () => {
		it('should parse stereo 16-bit info', () => {
			const audio = createSineWave(44100, 0.1, 440, 2, 16)
			const tta = encodeTta(audio)

			const info = parseTtaInfo(tta)
	...
	describe('encodeTta', () => {
		it('should encode sine wave', () => {
			...
			expect(isTta(tta)).toBe(true)
			expect(tta.length).toBeGreaterThan(100)
		})
		...
		it('should encode constant audio efficiently', () => {
			...
			// Simple predictor may not compress as well as full TTA
			expect(tta.length).toBeLessThan(8000)
		})

		it('should encode with format option', () => {
			const audio = createSineWave(44100, 0.1, 440, 2, 16)
			const tta = encodeTta(audio, { format: 1 })
	...
	describe('roundtrip', () => {
		it('should preserve sine wave samples exactly', () => {
			...
			// TTA is lossless - samples should be identical
		...
		it('should preserve 24-bit samples exactly', () => {
			const audio = createSineWave(48000, 0.05, 1000, 2, 16)
			const tta = encodeTta(audio)
			const decoded = decodeTta(tta)
	...
	describe('compression', () => {
		it('should compress audio significantly', () => {
			...
			// TTA should compress sine wave reasonably well
			expect(tta.length).toBeLessThan(rawSize)
		})
		...
		it('should achieve high compression on constant audio', () => {
			...
			// Constant audio should compress very well with simple predictor
			expect(tta.length).toBeLessThan(rawSize)
		})
	})
})
