/**
 * F4V (Flash Video MP4) encoder
 * Creates F4V files with MJPEG video
 */

import type { VideoData } from '@sylphx/codec-core'
import { encodeJpeg } from '../jpeg'
import { F4vBoxType, F4vBrand, F4vHandlerType, type F4vEncodeOptions } from './types'

/**
 * Encode frames to F4V
 */
export function encodeF4v(frames: VideoData[], options: F4vEncodeOptions = {}): Uint8Array {
	if (frames.length === 0) {
		throw new Error('No frames to encode')
	}

	const { frameRate = 30, timescale = 30000, quality = 85, brand = 'f4v ' } = options
...
function buildMdia(
	...
	const hdlr = buildHdlr(F4vHandlerType.VIDEO)
