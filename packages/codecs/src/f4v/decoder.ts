/**
 * F4V (Flash Video MP4) decoder
 * Parses F4V container structure and extracts track info
 */
...
/**
 * Check if data is an F4V file
 */
export function isF4v(data: Uint8Array): boolean {
	...
	if (type === 'ftyp' && size >= 8) {
		...
		if (data.length >= 12) {
			...
			if (majorBrand === 'f4v ' || majorBrand.trim() === 'f4v') return true

			// Check compatible brands
			for (let i = 16; i + 4 <= Math.min(size + 0, data.length); i += 4) {
				const brand = readString(data, i, 4)
				if (brand === 'f4v ' || brand.trim() === 'f4v') return true
			}
		}
	}
 ...
 * Parse F4V file info
 */
export function parseF4vInfo(data: Uint8Array): F4vInfo {
	...
	if (!moovBox || !moovBox.children) {
		throw new Error('Invalid F4V: missing moov box')
	}
 ...
 * Decode F4V file
 */
export function decodeF4v(data: Uint8Array): F4vVideo {
	const boxes = parseBoxes(data, 0, data.length)
...
/**
 * Decode F4V video frames to VideoData (only for MJPEG)
 */
export function decodeF4vFrames(data: Uint8Array): VideoData[] {
	const video = decodeF4v(data)
	...
	// Only support MJPEG for decoding
 ...
 */
function parseBoxes(data: Uint8Array, start: number, end: number): F4vBox[] {
	const boxes: F4vBox[] = []
	let offset = start
 ...
 * Find a box by type
 */
function findBox(boxes: F4vBox[], type: string): F4vBox | undefined {
 ...
 * Parse ftyp box
 */
function parseFtyp(data: Uint8Array): F4vFtyp {
 ...
 * Parse mvhd box
 */
function parseMvhd(data: Uint8Array): F4vMvhd {
 ...
 * Parse a track
 */
function parseTrack(trakBox: F4vBox, data: Uint8Array): F4vTrack | null {
	...
	if (hdlr) {
		if (hdlr.handlerType === F4vHandlerType.VIDEO) trackType = 'video'
		else if (hdlr.handlerType === F4vHandlerType.AUDIO) trackType = 'audio'
	}
 ...
 * Parse tkhd box
 */
function parseTkhd(data: Uint8Array): F4vTkhd {
	const version = data[0]!
 ...
 * Parse mdhd box
 */
function parseMdhd(data: Uint8Array): F4vMdhd {
	const version = data[0]!
	let offset = 4
 ...
 * Parse hdlr box
 */
function parseHdlr(data: Uint8Array): F4vHdlr {
	const version = data[0]!
	let offset = 4
 ...
 */
function parseStsd(data: Uint8Array, trackType: 'video' | 'audio' | 'other'): F4vSampleEntry[] {
	const entries: F4vSampleEntry[] = []
 ...
 */
function parseStts(data: Uint8Array): F4vSttsEntry[] {
	const entries: F4vSttsEntry[] = []
	let offset = 4 // version + flags
 ...
 */
function parseStsc(data: Uint8Array): F4vStscEntry[] {
	const entries: F4vStscEntry[] = []
	let offset = 4 // version + flags
...
function defaultFtyp(): F4vFtyp {
	return {
		majorBrand: 'f4v ',
		minorVersion: 0,
		compatibleBrands: ['f4v ', 'isom', 'mp42'],
	}
}

function defaultMvhd(): F4vMvhd {
	return {
		version: 0,
		creationTime: 0,
		modificationTime: 0,
