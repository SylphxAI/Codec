...
/**
 * Encode ImageData to TIFF
 */
export function encodeTiff(image: ImageData, options?: EncodeOptions): Uint8Array {
 ...
 */
function writeInlineValue(writer: TiffWriter, tag: TagData): void {
 ...
 */
function writeOverflowValue(writer: TiffWriter, tag: TagData): void {
	const values = Array.isArray(tag.value) ? tag.value : [tag.value]
