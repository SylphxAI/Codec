...
/**
 * Decode ORF to ImageData
 */
export function decodeOrf(data: Uint8Array): ImageData {
	const orf = parseOrf(data)

	if (orf.ifds.length === 0) {
		throw new Error('No image data in ORF')
	}

	// ORF files typically have multiple IFDs:
	// IFD0: Full resolution image or thumbnail
	// IFD1: Preview/thumbnail
	// SubIFD: RAW data
	// For simplicity, decode the first IFD with actual image data
	const ifd = findBestIFD(orf.ifds)
	return decodeIFD(data, ifd, orf.littleEndian)
}

/**
 * Parse ORF structure
 */
export function parseOrf(data: Uint8Array): OrfImage {
	const reader = new OrfReader(data)

	// Read byte order
	const byteOrder = reader.readU16(0)
	let littleEndian: boolean

	if (byteOrder === ORF_LITTLE_ENDIAN) {
		littleEndian = true
	} else if (byteOrder === ORF_BIG_ENDIAN) {
		littleEndian = false
	} else {
		throw new Error('Invalid ORF byte order')
	}

	reader.littleEndian = littleEndian

	// Check magic number (ORF or TIFF)
	const magic = reader.readU16(2)
	const isOlympus = magic === ORF_MAGIC
	if (magic !== ORF_MAGIC && magic !== TIFF_MAGIC) {
		throw new Error(`Invalid ORF magic number: ${magic}`)
	}
	...
	while (ifdOffset !== 0 && ifdOffset < data.length) {
		...
		// Also check for SubIFDs
		const subIFDOffsets = getTagArray(ifd, Tag.SubIFDs)
		for (const subOffset of subIFDOffsets) {
			if (subOffset > 0 && subOffset < data.length) {
	...
	return { littleEndian, isBigTiff: false, ifds, isOlympus }
}

/**
 * Find the best IFD to decode (prefer larger images)
 */
function findBestIFD(ifds: IFD[]): IFD {
	let bestIFD = ifds[0]!
	let maxPixels = 0
 ...
 */
function readIFD(reader: OrfReader, offset: number): IFD {
 ...
 * Read an IFD entry
 */
function readIFDEntry(reader: OrfReader, offset: number): IFDEntry {
 ...
 */
function readSingleValue(reader: OrfReader, offset: number, type: TagType): number {
	switch (type) {
		case TagType.Byte:
...
function decodeIFD(data: Uint8Array, ifd: IFD, littleEndian: boolean): ImageData {
	const reader = new OrfReader(data, littleEndian)
	...
	for (let stripIdx = 0; stripIdx < numStrips && stripIdx < stripOffsets.length; stripIdx++) {
		...
		switch (compression) {
			...
			case Compression.OJPEG:
				// JPEG compression in RAW files would require JPEG decoder
				throw new Error('JPEG compression in ORF not supported in pure TypeScript decoder')
			default:
				throw new Error(`Unsupported ORF compression: ${compression}`)
		}
			...
			if (planarConfig === PlanarConfig.Chunky) {
				...
				switch (photometric) {
					...
					case Photometric.Palette: {
						...
						// TIFF/ORF color maps are 16-bit, stored as RRR...GGG...BBB...
					...
					case Photometric.CFA:
						// Color Filter Array (Bayer) - simple demosaic
						// For a proper implementation, this would need debayering
				...
				switch (photometric) {
					case Photometric.RGB:
						...
						g = pixels[planeSize + y * width + x] ?? 0
						b = pixels[planeSize * 2 + y * width + x] ?? 0
						if (samplesPerPixel >= 4) {
							a = pixels[planeSize * 3 + y * width + x] ?? 255
						}
						break
					default:
						r = g = b = pixels[y * width + x] ?? 0
				}
			}
