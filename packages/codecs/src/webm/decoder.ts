...
import {
	...
	type WebmBlock,
	type WebmCluster,
	type WebmDecodeResult,
	type WebmInfo,
	type WebmTrack,
} from './types'

/**
 * Check if data is WebM
 */
...
/**
 * Parse WebM info without full decode
 */
export function parseWebmInfo(data: Uint8Array): WebmInfo {
	const result = decodeWebm(data)
	return result.info
}
 ...
 * Decode WebM file
 */
export function decodeWebm(data: Uint8Array): WebmDecodeResult {
	const reader = new EbmlReader(data)
	...
	if (!ebmlHeader || ebmlHeader.id !== EbmlId.EBML) {
		throw new Error('Invalid WebM: missing EBML header')
	}
	...
	for (const child of headerChildren) {
		switch (child.id) {
			case EbmlId.DocType:
				...
				if (docType !== 'webm') {
					throw new Error(`Invalid WebM: DocType is "${docType}", expected "webm"`)
				}
				break
	...
	if (!segment || segment.id !== EbmlId.Segment) {
		throw new Error('Invalid WebM: missing Segment')
	}
	...
	for (const track of tracks) {
		if (track.type === WebmTrackType.VIDEO && track.video) {
			width = track.video.pixelWidth
			height = track.video.pixelHeight
			hasVideo = true

			// Calculate FPS from defaultDuration (in nanoseconds)
			if (track.defaultDuration) {
				fps = 1000000000 / track.defaultDuration
			}
		}
		if (track.type === WebmTrackType.AUDIO) {
			hasAudio = true
		}
	}
 ...
 */
export function decodeWebmToVideo(data: Uint8Array): VideoData {
	const result = decodeWebm(data)

	// Find video track
	const videoTrack = result.info.tracks.find((t) => t.type === WebmTrackType.VIDEO)
	if (!videoTrack) {
		throw new Error('No video track found in WebM file')
	}

	const width = result.info.width
	const height = result.info.height
	const fps = result.info.fps || 30

	// Collect all video blocks with timestamps
	const blocks: { timestamp: number; data: Uint8Array; keyframe: boolean }[] = []

	for (const cluster of result.clusters) {
		for (const block of cluster.blocks) {
			if (block.trackNumber === videoTrack.number) {
				blocks.push({
					// timestamp is in ticks, timestampScale is ns/tick
					// To convert to ms: ticks * (ns/tick) / 1000000 ns/ms
					timestamp: block.timestamp * result.info.timestampScale / 1000000,
					data: block.data,
	...
	if (blocks.length === 0) {
		throw new Error('No video frames found in WebM file')
	}
	...
	// For now, we cannot decode VP8/VP9/AV1 frames in pure TypeScript
	// Return placeholder frames with correct metadata
	const frames = blocks.map((block, i) => {
	...
	const totalDuration = frames.length > 0 ? frames[frames.length - 1]!.timestamp + frameDuration : 0
 ...
 */
function parseTracks(reader: EbmlReader, element: EbmlElement, tracks: WebmTrack[]): void {
	const children = reader.parseChildren(element)
 ...
 */
function parseTrackEntry(reader: EbmlReader, element: EbmlElement): WebmTrack | null {
	const track: Partial<WebmTrack> = {}
	const children = reader.parseChildren(element)
...
function parseCluster(
	...
	const cluster: WebmCluster = { timestamp: 0, blocks: [] }
 ...
 * Parse SimpleBlock
 */
function parseSimpleBlock(reader: EbmlReader, element: EbmlElement, clusterTimestamp: number): WebmBlock | null {
 ...
 * Parse BlockGroup
 */
function parseBlockGroup(reader: EbmlReader, element: EbmlElement, clusterTimestamp: number): WebmBlock[] {
