/**
 * MKA (Matroska Audio) encoder
 * Creates MKA files with PCM audio
 */

import type { AudioData } from '@sylphx/codec-core'
import { EbmlId, MkaTrackType, type MkaEncodeOptions } from './types'

/**
 * Encode audio to MKA
 */
export function encodeMka(audio: AudioData, options: MkaEncodeOptions = {}): Uint8Array {
	const {
	...
	// Encode audio to PCM
	const pcmData = encodePcm(audio, bitDepth, codecId)

	// Calculate duration in timescale units
	const sampleCount = audio.data.length / channels
	const totalDuration = Math.round((sampleCount / sampleRate) * timescale)

	// Calculate frame duration (samples per frame)
	const samplesPerFrame = Math.round(sampleRate * 0.02) // 20ms frames
	const frameDuration = Math.round((samplesPerFrame / sampleRate) * timescale)
	...
	for (let offset = 0; offset < pcmData.length; offset += bytesPerFrame) {
		const frameSize = Math.min(bytesPerFrame, pcmData.length - offset)
		frames.push(pcmData.slice(offset, offset + frameSize))
	}
	...
	// EBML Header
	parts.push(buildEbmlHeader('matroska'))

	// Segment (with content)
	parts.push(buildSegment(sampleRate, channels, bitDepth, codecId, frameDuration, totalDuration, timescale, frames))
 ...
 */
function encodePcm(audio: AudioData, bitDepth: number, codecId: string): Uint8Array {
	const isLittleEndian = codecId === 'A_PCM/INT/LIT'
	const isFloat = codecId === 'A_PCM/FLOAT/IEEE'

	const bytesPerSample = bitDepth / 8
	const output = new Uint8Array(audio.data.length * bytesPerSample)
	const view = new DataView(output.buffer)

	for (let i = 0; i < audio.data.length; i++) {
		const sample = audio.data[i]!
		const byteOffset = i * bytesPerSample

		if (isFloat) {
			// Float PCM
			if (bitDepth === 32) {
				view.setFloat32(byteOffset, sample, isLittleEndian)
			} else if (bitDepth === 64) {
				view.setFloat64(byteOffset, sample, isLittleEndian)
			}
		} else {
			// Integer PCM - clamp and scale from [-1, 1]
			const clamped = Math.max(-1, Math.min(1, sample))

			if (bitDepth === 8) {
				output[byteOffset] = Math.round((clamped + 1) * 127.5)
			} else if (bitDepth === 16) {
				...
				if (isLittleEndian) {
					output[byteOffset] = value & 0xff
					output[byteOffset + 1] = (value >> 8) & 0xff
					output[byteOffset + 2] = (value >> 16) & 0xff
				} else {
					output[byteOffset] = (value >> 16) & 0xff
					output[byteOffset + 1] = (value >> 8) & 0xff
					output[byteOffset + 2] = value & 0xff
				}
			} else if (bitDepth === 32) {
				const value = Math.round(clamped * 2147483647)
...
function buildSegment(
	...
	// Tracks
	elements.push(buildTracks(sampleRate, channels, bitDepth, codecId, frameDuration))

	// Clusters (one cluster per frame for simplicity)
	let timestamp = 0
	for (let i = 0; i < frames.length; i++) {
		elements.push(buildCluster(timestamp, 1, frames[i]!, true))
...
function buildInfo(timescale: number, duration: number): Uint8Array {
	...
	// WritingApp
	elements.push(buildStringElement(EbmlId.WritingApp, 'mconv MKA encoder'))
...
function buildTracks(
	...
	const trackEntry = buildAudioTrack(1, sampleRate, channels, bitDepth, codecId, frameDuration)
	return buildMasterElement(EbmlId.Tracks, trackEntry)
}

/**
 * Build audio TrackEntry
 */
function buildAudioTrack(
	...
	// TrackType (audio = 2)
	elements.push(buildUintElement(EbmlId.TrackType, MkaTrackType.AUDIO))
	// FlagEnabled
	...
	// Audio settings
	elements.push(buildAudioSettings(sampleRate, channels, bitDepth))

	return buildMasterElement(EbmlId.TrackEntry, concatArrays(elements))
}

/**
 * Build Audio settings element
 */
function buildAudioSettings(sampleRate: number, channels: number, bitDepth: number): Uint8Array {
	const elements: Uint8Array[] = []

	// SamplingFrequency
	elements.push(buildFloatElement(EbmlId.SamplingFrequency, sampleRate))
	// Channels
	elements.push(buildUintElement(EbmlId.Channels, channels))
	// BitDepth
	elements.push(buildUintElement(EbmlId.BitDepth, bitDepth))

	return buildMasterElement(EbmlId.Audio, concatArrays(elements))
}
