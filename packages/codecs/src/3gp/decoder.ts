/**
 * 3GP (3rd Generation Partnership Project) decoder
 * Parses 3GP container structure and extracts track info
 */
...
import {
	...
	type ThreeGPMdhd,
	type ThreeGPMvhd,
	type ThreeGPSampleEntry,
	type ThreeGPStscEntry,
	type ThreeGPSttsEntry,
	type ThreeGPTkhd,
	type ThreeGPTrack,
	type ThreeGPVideo,
} from './types'
...
/**
 * Check if data is a 3GP file
 */
export function is3GP(data: Uint8Array): boolean {
 ...
 * Parse 3GP file info
 */
export function parse3GPInfo(data: Uint8Array): ThreeGPInfo {
	...
	if (!moovBox || !moovBox.children) {
		throw new Error('Invalid 3GP: missing moov box')
	}
	...
	let frameRate = 15 // Default for 3GP
 ...
 * Decode 3GP file
 */
export function decode3GP(data: Uint8Array): ThreeGPVideo {
	const boxes = parseBoxes(data, 0, data.length)
	const info = parse3GPInfo(data)

	// Find mdat
	const mdatBox = findBox(boxes, 'mdat')
	const mdatData = mdatBox
		? data.slice(mdatBox.offset + 8, mdatBox.offset + mdatBox.size)
		: undefined
...
/**
 * Decode 3GP to VideoData
 */
export function decode3GPToVideo(data: Uint8Array): VideoData {
	const video = decode3GP(data)
	const info = video.info

	// 3GP typically uses H.263/H.264/MPEG-4 which require hardware decoders
	...
	return {
		...
		codec: info.videoTrack?.codec || 'unknown',
		hasAudio: info.hasAudio,
		// Frame data not available without codec-specific decoders
		frames: [],
	}
}
...
function getSampleInfo(
	...
	for (let chunkIndex = 0; chunkIndex < track.chunkOffsets.length; chunkIndex++) {
		...
		for (
			let i = 0;
			i < samplesInChunk && sampleIndex < track.sampleSizes.length;
			i++
		) {
 ...
 */
function parseBoxes(data: Uint8Array, start: number, end: number): ThreeGPBox[] {
	const boxes: ThreeGPBox[] = []
	let offset = start
 ...
 * Find a box by type
 */
function findBox(boxes: ThreeGPBox[], type: string): ThreeGPBox | undefined {
 ...
 * Parse ftyp box
 */
function parseFtyp(data: Uint8Array): ThreeGPFtyp {
 ...
 * Parse mvhd box
 */
function parseMvhd(data: Uint8Array): ThreeGPMvhd {
 ...
 * Parse a track
 */
function parseTrack(trakBox: ThreeGPBox, data: Uint8Array): ThreeGPTrack | null {
	...
	if (hdlr) {
		if (hdlr.handlerType === ThreeGPHandlerType.VIDEO) trackType = 'video'
		else if (hdlr.handlerType === ThreeGPHandlerType.AUDIO) trackType = 'audio'
	}

	// Find stbl
	const minfBox = findBox(mdiaBox.children, 'minf')
	const stblBox =
		minfBox && minfBox.children ? findBox(minfBox.children, 'stbl') : null
	...
	const sampleEntries =
		stsdBox && stsdBox.data ? parseStsd(stsdBox.data, trackType) : []
	...
	const { sampleCount, sampleSizes } =
		stszBox && stszBox.data
			? parseStsz(stszBox.data)
			: { sampleCount: 0, sampleSizes: [] }
	...
	const syncSamples =
		stssBox && stssBox.data ? parseStss(stssBox.data) : undefined
	...
	return {
		...
		channelCount:
			trackType === 'audio' ? sampleEntries[0]?.channelCount : undefined,
		sampleEntries,
		sampleCount,
		sampleSizes,
		chunkOffsets,
 ...
 * Parse tkhd box
 */
function parseTkhd(data: Uint8Array): ThreeGPTkhd {
	const version = data[0]!
 ...
 * Parse mdhd box
 */
function parseMdhd(data: Uint8Array): ThreeGPMdhd {
	const version = data[0]!
	let offset = 4
 ...
 * Parse hdlr box
 */
function parseHdlr(data: Uint8Array): ThreeGPHdlr {
	const version = data[0]!
	let offset = 4
...
function parseStsd(
	...
	const entries: ThreeGPSampleEntry[] = []
	...
	for (let i = 0; i < entryCount && offset + 8 <= data.length; i++) {
		...
		const entry: ThreeGPSampleEntry = {
			format,
 ...
 */
function parseStts(data: Uint8Array): ThreeGPSttsEntry[] {
	const entries: ThreeGPSttsEntry[] = []
	let offset = 4 // version + flags
 ...
 */
function parseStsc(data: Uint8Array): ThreeGPStscEntry[] {
	const entries: ThreeGPStscEntry[] = []
	let offset = 4 // version + flags
...
function defaultFtyp(): ThreeGPFtyp {
	return {
		majorBrand: '3gp6',
		minorVersion: 0,
		compatibleBrands: ['3gp6', '3gp5', 'isom'],
	}
}

function defaultMvhd(): ThreeGPMvhd {
	return {
		version: 0,
		creationTime: 0,
		modificationTime: 0,
